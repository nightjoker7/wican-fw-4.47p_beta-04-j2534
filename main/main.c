...
//        		num_msg = 0;
		...
		// }
		static uint32_t j2534_rx_count = 0;
		static uint32_t j2534_rx_last_log = 0;
...
//        	num_msg++;
        	...
        	// Always call j2534_can_to_msg when J2534 is active (regardless of config protocol)
        	// This ensures CAN frames are buffered for J2534 READ_MSGS even when device
        	// is configured for a different protocol (e.g., OBD_ELM327)
        	if(j2534_is_active())
        	{
        		j2534_rx_count++;
        		// Log every CAN frame received while J2534 is active
        		ESP_LOGI(TAG, "[J2534-RX] Frame #%lu ID=0x%lX DLC=%d data=%02X %02X %02X %02X %02X %02X %02X %02X",
        		         j2534_rx_count, rx_msg.identifier, rx_msg.data_length_code,
        		         rx_msg.data[0], rx_msg.data[1], rx_msg.data[2], rx_msg.data[3],
        		         rx_msg.data[4], rx_msg.data[5], rx_msg.data[6], rx_msg.data[7]);
        		j2534_can_to_msg(&rx_msg, ucTCP_TX_Buffer.ucElement);
        	}
        	
        	//TODO: optimize, useless ifs
			if(tcp_port_open() || ble_connected() || HARDWARE_VER == WICAN_USB_V100 || mqtt_connected() || protocol == AUTO_PID || protocol == J2534_PROTO)
			{
				...
				{
					// Already handled above with j2534_is_active() check
					ucTCP_TX_Buffer.usLen = 0;
				}
				#if HARDWARE_VER != WICAN_PRO
				else if(protocol == OBD_ELM327 || protocol == AUTO_PID)
...
//		int can_datarate = config_server_get_can_rate();
	...
	{
		// j2534_init already called unconditionally above
		can_enable();
	}
	#if HARDWARE_VER == WICAN_PRO
